...
S["cover"]=(f'<section id="cover" data-transition="push" style="background:{K}; color:{W}; font-family:{BODY}; padding:128px; display:flex; flex-direction:column; justify-content:space-between">'
 ...
 f'<p style="font-size:26px; color:{GREY}">Executive and team briefing · September 2026 · v3.0</p></div>'
...
S["onepage"]=content("onepage","The plan on one page",
 ...
 +"".join(f'<div style="display:flex; flex-direction:column; gap:8px; background:{bg}; padding:22px 24px; border:2px solid {K}">'
   ...
   ("Target (to confirm)","Plan: $1.0M sale revenue incl. GST (about $915k net), the EE calendar plan of record for November is $990k. Floor: $585k (2025 repeated). Stretch: $1.27M on the Aug to Sep run rate. Spend 20 to 24% of revenue, ceiling 28%.","#ffffff"),
   ...
   ("What must be true first","Popup reach back above 50% on paid pages by 15 Oct. Pages live and tracked by 13 Nov. Bundle, DiggerShield and gift margins signed off by 30 Oct. Stock: $950k at RRP covers the plan only just (105% sell-through at $1.0M). Six decisions this week.","#ffffff")]) + '</div>',
...
S["target"]=content("target","Target: three cases on one basis (revenue incl. GST, as the EE calendar)",
 table(["Case","Sale revenue (2 hype + 14 days)","How it is derived","Meta spend at 20% / 24% / 28%"],[
  ...
  ["Plan: EE method","$1.0M (net about $915k)","10.9× the weekly BAU of Sep to Oct 2025 gave Nov 2025; applied to July 2026 BAU ($92.6k/wk) = $998k. The calendar&#39;s preloaded plan for November is $990k","$200k / $240k / $280k"],
 ...
 +f'<div style="display:flex; gap:24px">{big("1.72x","EOFY 2026 grew on EOFY 2025 while BAU grew 1.82x: the sale scales with BAU, which is why the multiple method holds.")}{big("$1.0M","is the plan of record in the EE calendar (Nov 2026 forecast $990k, +40% on Nov 2025 $707k). Profit on that plan: $174k in the calendar, before the cost corrections on the next slide.",bg="#ffffff")}{big("$118k","weekly BAU in Aug to Sep 2026 against $92.6k in July (the month the tool used). The choice of base month is the difference between $1.0M and $1.27M.")}</div>',
 src="EE calendar 2025 and 2026, Shopify, EE BFCM tool",notes="The earlier draft used the 2025 sale as the base case. That ignored 1.7x BAU growth since last spring; the EE multiple method is validated by EOFY 2026 and is now the plan case. The room chooses the base month.")

S["eeinputs"]=content("eeinputs","EE tool inputs: confirmed against the calendar and our data",
 table(["Input in the tool","Tool value","Calendar / Shopify value","Action"],[
  ["Sept and Oct 2025 revenue","$566,000","$566,323 (calendar, incl. GST)","Confirmed"],
  ["2025 BFCM event revenue","$700,000","$710,170 = the whole of November; the 14-day sale was $585k","Decide: month basis (10.9x) or event basis (9.2x, projection $850k on July BAU)"],
  ["Weekly BAU revenue","$92,641","July 2026 $417k / 4.5 wk = $92.7k; Aug to Sep run rate $118k","Decide the base month"],
  ["BAU MER","25.94%","Jul 39%, Aug 30%, Sep 28%, 2026 to date 27.5%","Correct; source not found"],
  ["Sale MER target","20%","Nov 2025 actual 24% ($167.7k on $710k)","Plan at 20 to 24%, ceiling 28%"],
  ["Product cost %","25%","Calendar driver 32% landed; 2026 actual 33% of revenue","Correct to 32%"],
  ["GST","8%","Driver 9%; 2026 actual 8.6%","Correct to 9%"],
  ["Daily fixed costs","$2,414","$83,905/month from Aug 2026 = $2,707/day (preloaded year uses it)","Correct to $2,707"],
  ["Packaging / shipping / merchant","$2 / $24 / 2%","$1.50 / $24 / 2%","Minor"],
  ["AOV","$279.83","Jul $252, Aug $316, 2026 to date $302; Nov 2025 $328","Use $300 to $320"],
  ["Daily BAU spend bracket","$500 to $3K","$4.6k to $5.2k per day Jul to Sep","Set to $3K+"],
  ["Repeat rate","Hybrid","Returning orders 24% (calendar), 21.4% returning-customer rate","Confirmed, on the line"]],[24,14,40,22],size=22),
 src="EE calendar 2025 and 2026, Shopify, EE BFCM tool",notes="Every line was checked against the two calendar workbooks or Shopify. The four corrections that move money are product cost, fixed costs, the base month and the event-versus-month basis.")
...
S["unit"]=content("unit","Profit per sale order: the tool&#39;s number and the corrected number",
 table(["Per sale order (with gift, 40% / 20% uptake)","EE tool","Corrected (product 32%, GST 9%, fixed $2,707/day)"],[
  ["Revenue per order","$279.72","$279.72 (use $300+ if AOV holds at 2025 sale levels)"],
  ["GST","$20.72 (7.4%)","$23.10 (9% of ex-GST base, est.)"],
  ["Meta at 20% MER","$55.94","$55.94 (at 24%: $67.13)"],
  ["Variable costs incl. gifts","$133.24 (47.6%)","about $152.80 (54.6%): +7 points of product cost"],
  ["Fixed costs per order","$10.83","$12.14"],
  ["Profit per order","$59.00 (21.1%)","about $35 (12.6%); about $24 (8.5%) at 24% MER"],
  ["Total profit on 3,568 orders","$210,475","about $126k; about $85k at 24% MER"]],[36,24,40],size=24)
 +note("Estimates: the tool&#39;s formula is not fully visible, so the corrected column moves each line by the input difference only. The direction is certain: the calendar&#39;s own November 2025 shows 15% profit on the month at 24% MER, and 2026 fixed costs are 12% higher than July."),
 src="EE BFCM tool section 7, EE calendar drivers",notes="This is why the input corrections matter: the sale is still profitable, at about half the margin the tool shows.")
...
S["paid"]=content("paid","Paid media plan: the EE budget and how to shape it",
 f'<div style="display:flex; gap:24px; flex:1">'
 +card("Budget (EE tool, 20% MER on $998k)",["Total $199,587: Meta $171,645 (86%), TikTok $19,959 (10%), Google $7,983 (4%)","Hype days $13,977 each; sale days $12,260 each (smoothed)","At 24% MER the same plan is $240k; ceiling 28% = $280k","Test markets (USA, NZ, TikTok, YouTube) sit inside the 14%, each with a day-4 kill rule"],accent=Y,lsize=24)
 +card("Meta structure (from the campaign plan)",["Sale campaigns $99k: TOF broad and ASC $32.5k, TOM (ASC, exclusions) $34.3k, MOF $19.6k, BOF $12.6k","Evergreen kept live $72.6k: TOF interest and broad $32.1k, Advantage+ $24.5k, MOF $11.8k, BOF $4.1k","Hype days spend only on TOM, MOF and BOF (warm); TOF starts on day 1","Cold TOF lands on grease and PRO Mat product pages; TOM/MOF/BOF on the sale page"],accent=K80,lsize=24)
 +card("Shape it to the curve",["Smoothed spend gives day-1 MER 4% and days 8 to 10 MER 41% with three loss days in the tool&#39;s own equation","Shaped (&#39;clunky&#39;) spend from the tool: $56k day 1, $24k day 2, $20k day 3, then $6k to $14k a day","Recommendation: shaped spend, 35 / 45 / 20 across launch, plateau and close","Daily 8 AM check with the cut rules on the measurement slide"],accent=Y,lsize=24)+'</div>',
 src="EE BFCM tool sections 8 and 9, meta-campaigns.csv",notes="The tool&#39;s smoothed spend produces negative-profit days in the plateau; its own clunky row fixes that. Shape the spend.")
...
S["offer"]=content("offer","Offer architecture",
 ...
 +card("Gift with purchase",["$299 spend: Magnet Tool Mat","$399 spend: free shipping","$599 spend: Drawbar Cover","Thresholds sit just above the $305 first-order average; the EE tool suggests $340 and $450 minimums for the two gifts at $14 cost each"],accent=K,tsize=32,lsize=26)
...
S["deliverables"]=content("deliverables","Creative deliverables and deadlines (proposal)",
 ...
 src="Planning notes, EE tool section 6",notes="The EE tool sizes the creative at 40 to 53 assets (18 to 24 video, 22 to 29 static): 4 to 9 for hype, 9 to 13 live at launch, four per mid-sale offer, extra for Black Friday and last chance. The counts above cover it. Hard constraint: 13 November.")
...
tl=[("Tue 17 and Wed 18 Nov","Hype","Two hype sends to engaged segments; warm-audience ads only (TOM, MOF, BOF). All hype traffic to the first-access capture page with a countdown."),
    ...
    ("Thu 19 Nov, 6:00 AM AEDT","Launch (day 1)","Sale live at 6:00 AM so day one is a full day (2025 launched at 3:05 PM). Launch email and SMS 7:00 AM. Cold traffic to product pages; 28% of sale revenue expected on day one."),
...
S["timeline"]=content("timeline","Timeline: 2 hype days plus 14 sale days, 19 Nov to 2 Dec, all times AEDT",
 ...
 +note("Decision made: 14 days from Thu 19 Nov. Three artefacts still carry other dates and need updating: the EE tool (18 Nov to 1 Dec), the email flow and Meta plan files (12 days from 19 Nov plus an extension day), and the shared calendar (23 Nov start). Hours from 2025: orders ran 85 to 100 an hour from 7 to 11 AM and 115 to 120 an hour from 6 to 9 PM.",size=22),
 src="Planning decision, EE tool, email-flow.csv, campaign calendar",notes="Black Friday lands on day 9 and Cyber Monday on day 12; the last two days run into December, so the last-chance push is Wednesday 2 December, not Cyber Monday.")
...
S["ops"]=content("ops","Stock, fulfilment and customer service (to confirm)",
 f'<div style="display:flex; gap:24px">{big("1,000","orders on day one at the $1.0M plan (28% of 3,570 sales orders), against 251 on launch day 2025. Fulfilment, packaging stock and customer service must be planned for four times last year&#39;s peak.",bg="#ffffff")}{big("$950k","stock at RRP against a $1.0M plan: the EE tool shows 105% sell-through and a stock-out risk. Gift units at 3,570 orders: about 1,500 magnet mats, 1,000 free-shipping orders and 525 drawbar covers on the 2025 order-value spread (the tool assumes 1,430 and 715 at 40% and 20% uptake).")}{big("[ ]","bundle kits pre-packed before launch: components for three hero bundles at the volumes the drops will drive.",bg="#ffffff")}</div>'
 +f'<div style="display:flex; gap:24px; flex:1">'
 +card("Stock and warehouse",["Gift-with-purchase stock reserved by 10 Nov: about 1,500 magnet mats and 525 drawbar covers at the plan, more at stretch","Stock-out plan: which SKUs come off the sale first, and the substitute gift when magnet mats run out","Bundle components kitted; a stop rule when any component runs out","Drop-of-the-day stock confirmed the day before each drop","Warehouse shoot day (6 Nov) doubles as the stock check"],accent=Y)
...
S["measure"]=content("measure","Measuring the sale: the 8 AM scorecard and the cut rules",
 table(["Row","Target or benchmark","Rule if missed","Source"],[
  ...
  ["Meta spend to revenue","20 to 24% (EE plan 20%; 2025 actual 24%); ceiling 28%","Over 30% for two days: cut broad prospecting","Meta, Shopify"],
...
S["decisions"]=content("decisions","Decisions needed this week",
 f'<div style="display:grid; grid-template-columns:repeat(3, 1fr); gap:20px; flex:1">'+"".join(f'<div style="display:flex; flex-direction:column; gap:8px; background:{"#ffffff" if i%2 else Y40}; padding:22px 24px; border:2px solid {K}"><h3 style="font-family:{HEAD}; font-size:28px; font-weight:700; text-transform:uppercase">{t}</h3><p style="font-size:24px; line-height:1.3; color:{K90}">{d}</p></div>' for i,(t,d) in enumerate([
  ("Target","Plan $1.0M incl. GST (EE plan of record) or stretch $1.27M on the current run rate; spend 20 to 24%, ceiling 28%."),
  ("EE tool corrections","Product cost 32%, GST 9%, fixed $2,707/day, base month, event-versus-month basis; re-date the tool and files to 19 Nov to 2 Dec."),
...
S["faq-exec"]=content("faq-exec","Questions the exec will ask",
 f'<div style="display:flex; gap:32px; flex:1"><div style="flex:1">'+qa([
  ("What are we targeting and what does it cost?","Plan $1.0M sale revenue incl. GST, the EE calendar plan of record, at $200k to $240k of spend (20 to 24%). Floor $585k if we only repeat 2025; stretch $1.27M on the current run rate. Profit at the plan is about $126k after correcting the tool&#39;s cost inputs, not the $210k the tool shows."),
  ...
  ("What stops us overspending?","A 28% ceiling with a 20 to 24% plan, spend shaped to the revenue curve rather than smoothed (the tool&#39;s smoothed plan produces three loss days), written cut rules, and one person reading the scorecard at 8 AM. Cold traffic no longer goes to the sale page.")],qsize=26,asize=23)+'</div><div style="flex:1">'+qa([
